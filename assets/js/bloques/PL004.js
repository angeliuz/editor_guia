class PL004 {
  ...
  render(){
      ...
      html+='             <div data-texto="texto" class="editable col-12 fs-5 f-nunito fw-400 color-2E ps-20 mb-2 justify-content-start text-input" contenteditable="true">';
  ...
  renderOpcion(opcion){
    return '<li class="color-CC opcion"><div class="handle"><i class="fas fa-th"></i></div><span class="color-2E"><div class="editable li-editable" contenteditable="true" data-docid="'+opcion.doc_id+'">'+opcion.texto+'</div></span><div class="remove-li" data-docid="'+opcion.doc_id+'"><i class="fas fa-times"></i></div></li>';
  }
  ...
  initListeners(){
    ...
    let inputs = this.bloque.find(".text-input");
      for(let i=0; i<inputs.length; i++){
        ...
        new MediumEditor($(inputs[i]),EditorTexto.optionsEditor);
    ...
    let li_editables = this.bloque.find(".li-editable");
      ...
      for(let i=0; i<li_editables.length; i++){
        ...
        new MediumEditor($(li_editables[i]),EditorTexto.optionsEditor);
      }
    ...
    for(let i=0; i<addButtons.length; i++){
      ...
      $(addButtons[i]).click(function(){
        ...
        parent.opciones_db.add(data).then((op)=>{
              ...
              new MediumEditor($(input[0]),EditorTexto.optionsEditor);
  ...
  addClickRemove(button){
    ...
    $(button).click(function(){
        ...
        li.html('Eliminando...');
        li.css('opacity','.5');
        parent.opciones_db.doc(doc_id).delete().then((doc) =>{
  ...
  ordenar(lis,cb){
    ...
    function update(){
      ...
      opciones_db.doc(op_id).update(data).then((doc) =>{
