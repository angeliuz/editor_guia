class Imagen {
      ...
      if(btn_right){
       ...
       if(btn_delete){
        ...
        if(drag_divs){
          ...
          let btn_color_globo = $(drag_divs).find('.btn_color_globo')[0];
         /* $(drag_divs).css('left',($(columna).width()-$(drag_divs).width())/2);
          ...
          $(btn_color_globo).click(function(){
            ...
            for(let i=0; i<clases.length;i++){
              if($(drag_divs).hasClass(clases[i])){
                indice = i;
              }
          ...
          rangeGlobo.addEventListener('input', function () {
          ...
          rangeGlobo.addEventListener('change', function () {
  ...
  renderColorGlobo(){
        return '<div class="btn_color_globo"><i class="fas fa-fill-drip"></i></div>';
  }
  ...
  renderControlGlobo(data){
    ...
    html+=this.renderColorGlobo(data)
    html+= '</div>';
    return html;
  }
