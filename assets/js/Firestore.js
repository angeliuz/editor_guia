let Firestore = {
    ...
    init:function(){
        ...
        Firestore.paginas_db = db.collection('libros').doc('fi500Vx5J4vP4BXV5tez').collection('paginas');
        //Firestore.paginas_db = db.collection('libros').doc('IMRk3NeCu9nwj1mvCyt1').collection('paginas');
    ...
    deleteItem:function(pagina_id,bloque_id,cb){
        let bloques_db = Firestore.paginas_db.doc(pagina_id).collection(Firestore.bloques_name);
        bloques_db.doc(bloque_id).delete().then((doc) =>{
          cb(doc);
        });
    },
    addItem:function(pagina_id,data,opciones,cb){
        ...
        bloques_db.add(data).then((doc) =>{
          ...
          if(opciones){
            let ids = []
            bloques_db.doc(doc.id).collection('opciones').add(opciones[0]).then((op)=>{
                ids.push(op.id);
                bloques_db.doc(doc.id).collection('opciones').add(opciones[1]).then((op)=>{
                    ids.push(op.id);
                    bloques_db.doc(doc.id).collection('opciones').add(opciones[2]).then((op)=>{
                        ids.push(op.id);
                        cb(doc_item,ids);
                    })
                })
